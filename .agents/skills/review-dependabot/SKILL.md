...
After merge, use skill `finish-pr` only if the PR needs conflict/CI/comment follow-up on a feature branch — not for routine weekly bumps on `develop`.
...
| `finish-pr`        | Feature PR stuck on CI/comments — not standard Dependabot flow |
